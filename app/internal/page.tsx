...
export default function InternalPage() {
  ...
  const { t } = useLanguage();
  const mockInternal = mockCorrespondence.filter(c => c.type === 'internal');
  const uploadedInternal = getDocumentsByType('internal');
  const allInternal = [...mockInternal, ...uploadedInternal];
  ...
  const handleUpload = (data: { department: string; topic: string; files: File[]; role?: string; category?: string; uploadedFiles?: Array<{ file: File; filePath: string; fileName: string; fileSize: number; fileType: string }> }) => {
    ...
    filesToProcess.forEach((fileData, index) => {
      const newDoc: Document = {
        id: `internal-${Date.now()}-${index}`,
        type: 'internal',
        title: data.topic,
        topic: data.topic,
        description: `${t.pages.internal.title}: ${data.topic}`,
  ...
  const filteredInternal = allInternal.filter(item => {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-1">
            {t.pages.internal.title}
          </h1>
          <p className="text-sm text-gray-500">{t.pages.internal.description}</p>
        </div>
        <button 
          onClick={() => setShowUploadModal(true)}
          className="flex items-center gap-2 rounded-lg bg-purple-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-purple-700 hover:shadow-md"
        >
          <Upload className="h-4 w-4" />
          {t.pages.internal.upload}
        </button>
      </div>
      ...
      <div className="mb-6 flex gap-3 flex-wrap">
        <input
          ...
          placeholder={t.pages.internal.searchTopic}
          className="flex-1 min-w-[200px] rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm shadow-sm transition-all focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-100 placeholder:text-black"
        />
        <input
          type="text"
          value={selectedDocumentNumber}
          onChange={(e) => setSelectedDocumentNumber(e.target.value)}
          placeholder={t.pages.internal.searchDocNumber}
          className="flex-1 min-w-[200px] rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm shadow-sm transition-all focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-100 placeholder:text-black"
        />
        <select 
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredInternal.length > 0 ? (
          filteredInternal.map((correspondence) => {
      ...
      <DocumentUploadModal
        ...
        title={t.pages.internal.upload}
        description={t.pages.internal.description}
        color="purple"
      />
