...
export default function IncomingPage() {
  ...
  const { t } = useLanguage();
  const mockIncoming = mockCorrespondence.filter(c => c.type === 'incoming');
  const uploadedIncoming = getDocumentsByType('incoming');
  const allIncoming = [...mockIncoming, ...uploadedIncoming];
  ...
  const handleUpload = (data: { department: string; topic: string; files: File[]; role?: string; category?: string; uploadedFiles?: Array<{ file: File; filePath: string; fileName: string; fileSize: number; fileType: string }> }) => {
    ...
    filesToProcess.forEach((fileData, index) => {
      const newDoc: Document = {
        ...
        title: data.topic,
        topic: data.topic,
        description: `${t.pages.incoming.title}: ${data.topic}`,
        ...
        status: 'pending',
        date: new Date(),
        documentNumber: `IN-${new Date().getFullYear()}-${String(allIncoming.length + index + 1).padStart(3, '0')}`,
    ...
    setToast({ message: `${filesToProcess.length} ${t.toast.uploadSuccess}`, type: 'success' });
  };

  // Filter by department, topic, document number, and category
  const filteredIncoming = allIncoming.filter(item => {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-1">
            {t.pages.incoming.title}
          </h1>
          <p className="text-sm text-gray-500">{t.pages.incoming.description}</p>
        </div>
        <button 
      ...
      <div className="mb-6 flex gap-3 flex-wrap">
        <input
          ...
          placeholder={t.pages.incoming.searchTopic}
          className="flex-1 min-w-[200px] rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm shadow-sm transition-all focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 placeholder:text-black"
        />
        <input
          type="text"
          value={selectedDocumentNumber}
          onChange={(e) => setSelectedDocumentNumber(e.target.value)}
          placeholder={t.pages.incoming.searchDocNumber}
          className="flex-1 min-w-[200px] rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm shadow-sm transition-all focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 placeholder:text-black"
        />
        <select 
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredIncoming.length > 0 ? (
          filteredIncoming.map((correspondence) => {
      ...
      <DocumentUploadModal
        ...
        title={t.pages.incoming.upload}
        description={t.pages.incoming.description}
        color="blue"
      />
