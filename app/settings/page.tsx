...
import { Users, Building2, Shield, Plus, Edit, Trash2, Search, UserPlus, ShieldPlus, Check, X, Eye, FileEdit, Settings as SettingsIcon, Briefcase } from 'lucide-react';

export default function SettingsPage() {
  const [activeSection, setActiveSection] = useState<'users' | 'departments' | 'permissions' | 'companies' | null>(null);
  ...
  const [users, setUsers] = useState([
    { id: '1', name: 'أحمد محمد', email: 'ahmed@example.com', department: 'Technical', role: 'مدير', status: 'نشط' },
    { id: '2', name: 'فاطمة علي', email: 'fatima@example.com', department: 'Security', role: 'كاتب', status: 'نشط' },
    { id: '3', name: 'محمد خالد', email: 'mohammed@example.com', department: 'Commercial', role: 'مشاهد', status: 'معطل' },
  ]);

  const [departments, setDepartments] = useState([
    { id: '1', name: 'الإدارة الفنية', code: 'Technical', manager: 'أحمد محمد', usersCount: 12, parent: null },
    { id: '2', name: 'الإدارة الأمنية', code: 'Security', manager: 'فاطمة علي', usersCount: 8, parent: null },
    { id: '3', name: 'الإدارة التجارية', code: 'Commercial', manager: 'محمد خالد', usersCount: 15, parent: null },
    { id: '4', name: 'إدارة القبطانية', code: 'Captaincy', manager: 'سارة أحمد', usersCount: 5, parent: null },
  ]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50/30 to-purple-50/30 p-8">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        {/* Companies Card */}
        <div className={`group relative overflow-hidden rounded-2xl border-2 ${activeSection === 'companies' ? 'border-purple-400' : 'border-gray-200'} bg-gradient-to-br from-white to-purple-50/50 p-6 shadow-lg transition-all duration-300 hover:shadow-xl`}>
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 font-medium">{companies.length} شركة</p>
              </div>
            </div>
            <button
          ...
          <div className="space-y-2 mb-4">
            {companies.slice(0, 3).map((company) => (
              <div key={company.id} className="flex items-center justify-between p-2 bg-white rounded-lg border border-gray-200">
                <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="w-full rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 px-4 py-3 text-white font-semibold shadow-md transition-all duration-300 hover:scale-105 hover:shadow-lg"
          >
            {activeSection === 'companies' ? 'إخفاء القائمة' : 'عرض جميع الشركات'}
          </button>
        </div>
      </div>
      ...
      {activeSection === 'companies' && (
        <div className="mt-8 rounded-2xl border-2 border-purple-200 bg-white p-6 shadow-lg">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-900">قائمة الشركات</h2>
            <button
              onClick={() => setShowCompanyModal(true)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {companies.map((company) => (
              <div key={company.id} className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">رمز: {company.code}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-mono text-xs bg-gray-100 px-2 py-1 rounded">
                      {company.password || 'غير محدد'}
                    </span>
                  </div>
                  <div>
                    <span className="font-semibold">رقم الهاتف:</span> {company.phone || 'غير محدد'}
                  </div>
                  <div>
                    <span className="font-semibold">المسؤول عن المراسلات:</span> {company.contactPerson || 'غير محدد'}
                  </div>
                  <div>
                    <span className="font-semibold">الحالة:</span> 
                    <span className={`ml-2 px-2 py-1 rounded-full text-xs ${
                      company.status === 'نشطة' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
                      {company.status}
                    </span>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select className="w-full px-4 py-2 bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 rounded-lg text-black focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-400">
                  <option>الإدارة الفنية</option>
                  <option>الإدارة التجارية</option>
                  <option>الإدارة الأمنية</option>
                  <option>إدارة القبطانية</option>
                </select>
              </div>
              <div>
      ...
      {showCompanyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="relative w-full max-w-md bg-white rounded-xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">إضافة شركة جديدة</h2>
              <button onClick={() => setShowCompanyModal(false)} className="p-2 text-gray-400 hover:text-gray-600">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">رمز الشركة</label>
                <input type="text" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">NIF (رقم التعريف الضريبي)</label>
                <input type="text" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">البريد الإلكتروني</label>
              ...
              <div>
                ...
                <input type="tel" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">اسم المسؤول عن المراسلات</label>
                <input type="text" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">الحالة</label>
                <select className="w-full px-4 py-2 bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-200 rounded-lg text-black focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-400">
                  <option>نشطة</option>
                  <option>معطلة</option>
                </select>
              </div>
            ...
            <div className="flex items-center gap-3 mt-6">
              <button onClick={() => setShowCompanyModal(false)} className="flex-1 px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
                إلغاء
              </button>
              <button onClick={() => setShowCompanyModal(false)} className="flex-1 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
                إضافة
              </button>
            </div>
          </div>
